...
class Report:
    def __init__(self,root):
        ...
        # =============== title ==================
        lbl_title=Label(self.root,text="ROOM BOOKING REPORT", font=("times new roman",18, "bold"),bg="lightsteelblue", fg="red",bd=4,relief=RIDGE)
        ...
        btnReset=Button(btn_frame,text="Reset",command=self.reset_data,font=("arial",12,"bold"),bg="lightsteelblue",fg="red",width=9)
        btnReset.grid(row=0,column=3,padx=1)

        # ============== table frame search system =============
        Table_Frame=LabelFrame(self.root,bd=2,relief=RIDGE,text="Report Details",font=("arial",12,"bold"),padx=2)
    ...
    def update(self):
        if self.var_floor.get()=="" or self.var_roomNo.get()=="":
            messagebox.showerror("Error","Please enter Floor number and Room No.",parent=self.root)
        else:
            conn=mysql.connector.connect(host="localhost",username="root",password="Sir_Levi@7",database="management")
            my_cursor=conn.cursor()
